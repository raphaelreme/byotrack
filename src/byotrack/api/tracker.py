...
class PauseableTQDM(tqdm.tqdm):
    ...
    def update(self, n: float | None = 1) -> bool | None:
        if self.last_pause_t != 0.0:
            warnings.warn(
                "Progress bar is paused. You should explicitly call unpause before updating again", stacklevel=2
            )
            self.unpause()
    ...
    def pause(self, *, refresh=True):
        ...
        if self.last_pause_t != 0.0:
            warnings.warn("The progress bar was already paused", stacklevel=2)
            return

        if refresh:  # By default refresh before doing a pause
            self.refresh()

        self.last_pause_t = self._time()  # type: ignore[attr-defined]
    ...
    def unpause(self):
        ...
        if self.last_pause_t == 0.0:
            warnings.warn("The progress bar was not paused", stacklevel=2)
            return

        delta_t = self._time() - self.last_pause_t  # type: ignore[attr-defined]
        self.last_pause_t = 0.0
        if delta_t < 0.0:
            warnings.warn("Found negative pause time, ignoring...", stacklevel=2)
            return
...
class MultiStepTracker(Tracker):
...
class BatchMultiStepTracker(MultiStepTracker):
