...
class OpticalFlow(ABC):
    ...
    Attributes:
        downscale (float | np.ndarray): Downscale factor. Can be provided for each spatial axis.
            Default: 2.0
        blur (None | float | np.ndarray): Std of the Gaussian blur applied before downscaling the images
            Can be provided for each spatial axis. If not provided, a default one is chosen given the downscale.

    """

    def __init__(self, downscale: float | np.ndarray = 2.0, blur: None | float | np.ndarray = None):
    ...
    def transform(self, flow_map: np.ndarray, points: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: New positions of the points ([k, ]i, j) in the original (non-downscale) coordinates
    ...
    def warp(self, flow_map: np.ndarray, moving: np.ndarray) -> np.ndarray:
        """Warp the moving image onto the reference using the flow map.
