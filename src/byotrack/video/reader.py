...
class MetaVideoReader(type):
    ...
    extension_to_reader: ClassVar[dict[str, MetaVideoReader]] = {}
...
class VideoReader(metaclass=MetaVideoReader):
    ...
    * Easy to check main attributes like:
    ...
    def __init__(self, path: str | os.PathLike, **kwargs: Any):  # noqa: ARG002
...
class TiffVideoReader(VideoReader):
    ...
    def __init__(  # noqa: D417
        ...
        **kwargs: Any,
        ...
        if self.out_axes[-1] != "C":
            raise ValueError(f"Channels have to be the last out axes. Found {self.out_axes}.")

        self.video = tifffile.TiffFile(path, **kwargs).series[0].levels[level]
        self.level = level
    ...
    @staticmethod
    def _check_slice(ax_slice: dict[str, slice], axes: str) -> None:
        if "T" in ax_slice:
    ...
    def _extract_axes(self, axes: str | None = None) -> dict[str, int]:  # noqa: C901
        if axes:
            # Check that the provided axes are valid
            assert len(set(axes)) == len(axes), "Duplicated axes"
            assert "T" in axes and "X" in axes and "Y" in axes, "Axes should include T, X and Y"  # noqa: PT018
            assert len(axes) <= 5, "The reader can only handle up to 3D + T + C"  # noqa: PLR2004
            for axis in axes:
        ...
        assert 3 <= len(in_axes) <= 5, "The reader expects at least 2D + T and at most 3D + T + C"  # noqa: PLR2004
        assert "X" in in_axes and "Y" in in_axes, "Missing a spatial dimension X or Y"  # noqa: PT018
    ...
    @property
    def axes(self) -> Iterable[str]:  # noqa: D102
    ...
    #     shape = self.video._shape_squeezed
    ...
    def _load(self):
        ...
        for index in np.ndindex(*self._current.shape[: -self._page_dim]):
            ...
            self._current[index] = self.video[i].asarray()[slices[-self._page_dim :]]
...
class FrameTiffLoader:
    ...
    Note:
        ...
        (T, I, O, Q, ...) is either interpreted as Z if it is missing, or it will
        yield an error.
    ...
    def __init__(self, level=0, axes: str | None = None, ax_slice: dict[str, slice] | None = None):
        self.level = level
        self.axes = axes
        self.ax_slice = {} if ax_slice is None else ax_slice

        TiffVideoReader._check_slice(self.ax_slice, self.out_axes)  # noqa: SLF001
        ...
        if self.axes:
            ...
            assert "X" in self.axes and "Y" in self.axes, "Missing a spatial dimension X or Y"  # noqa: PT018
            assert len(self.axes) <= 4, "The frame loader can only load up to 3D + C images"  # noqa: PLR2004
            for axis in self.axes:
                assert axis in self.out_axes, f"Unknown axis: {axis} not in {self.out_axes}"

    def _extract_axes(self, series: tifffile.TiffPageSeries) -> dict[str, int]:
        if self.axes:
        ...
        assert "X" in axes and "Y" in axes, "Missing a spatial dimension X or Y"  # noqa: PT018
        assert len(series.axes) <= 4, "The frame loader can only load up to 3D + C images"  # noqa: PLR2004
    ...
    def __call__(self, path: str | os.PathLike) -> np.ndarray:  # noqa: D102
        tiff = tifffile.TiffFile(path)
        series = tiff.series[0].levels[self.level]
        axes = self._extract_axes(series)
        ...
        if not self.ax_slice:
            ...
            output = self._read_slice(series, slices)
    ...
    @staticmethod
    def _read_slice(series: tifffile.TiffPageSeries, slices: tuple[slice, ...]) -> np.ndarray:
        ...
        output = np.zeros(final_shape, dtype=series.keyframe.dtype)

        for index in np.ndindex(*final_shape[:-page_dim]):
            i = int(offset + np.sum(np.array(index) * strides))
            output[index] = series[i].asarray()[slices[-page_dim:]]  # type: ignore[union-attr]
...
class MultiFrameReader(VideoReader):
    ...
    Attributes:
        ...
        frame_loader (Callable[[str | os.PathLike], np.ndarray]): Loads frame from their associated files.
