        ...
        .. code-block:: xml
            ...
            <root>
                <MHTconfiguration>
                    ...
                    <output trackGroupName="mht-tracks-1"/>
                </MHTconfiguration>
            </root>
...
class IcyEMHTLinker(byotrack.Linker):
    ...
    def run(self, video, detections_sequence: Sequence[byotrack.Detections]) -> list[byotrack.Track]:
        try:
            ...
            if pathlib.Path(self.rois_file).exists():
                pathlib.Path(self.rois_file).unlink()
    ...
    def _run_icy(self):
        ...
        if self.full_specs:
            ...
            self.runner.run(
                ...
                rois=f'"{pathlib.Path(self.rois_file).resolve()}"',
                parameters=f'"{pathlib.Path(self.parameters_file).resolve()}"',
                tracks=f'"{pathlib.Path(self.tracks_file).resolve()}"',
            )
        else:
