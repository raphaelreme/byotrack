...
class TrackHandler:
    ...
    the following associated detections. At the beginning, the track is considered HYPOTHETICAL.
    ...
    def is_active(self) -> bool:
        """Check whether the track is still active (Hypothetical or Valid)."""
        return self.track_state <= self.TrackState.VALID
    ...
    def update(self, frame_id: int, detection_id: int) -> None:
        ...
        if not self.is_active():
            raise RuntimeError("An inactive track cannot be updated")
        if len(self.track_ids) != len(self.detection_ids):
            raise RuntimeError("The linker should call `update` then `register_track_id` at each linking step")
        if self.start + len(self.detection_ids) != frame_id:
            raise RuntimeError("The linker should update each active track on each time frame.")
        ...
        if detection_id == -1:  # Not associated
            ...
            if self.track_state == TrackHandler.TrackState.HYPOTHETICAL and len(self.detection_ids) >= self.n_valid:
...
class OnlineFlowExtractor:
    ...
    def __init__(self, optflow: byotrack.OpticalFlow) -> None:
        ...
        self.flow_map: np.ndarray | None = None
...
class FrameByFrameLinker(byotrack.OnlineLinker):
    ...
    Attributes:
        ...
        optflow (OnlineFlowExtractor | None): Optional wrapper around the given optional OpticalFlow that
            will extract flow maps of the video online. (The underlying OpticalFlow object is accessible in
            self.optflow.optflow)
            Default: None
        features_extractor (FeaturesExtractor | None): Optional features extractor that will extract
            ...
            points = torch.cat(
                [
                    ...
                    for track_id, positions in zip(
                        handler.track_ids[: len(handler)], self.all_positions[handler.start :], strict=False
                    )
                ]
            )
    ...
    def update_active_tracks(self, detections: byotrack.Detections) -> torch.Tensor:  # noqa: C901, PLR0912, PLR0915
        ...
        active_mask = torch.full((len(self.active_tracks),), fill_value=False)
        ...
        for i, track in enumerate(self.active_tracks):
            if track_to_det_split[i] != -1:  # The track splits
                ...
                # Swap other with track (the split one is finished)
                track = other  # noqa: PLW2901

            if track_to_det[i] == -1 and track_to_det_merge[i] != -1:  # The track is the second branch of a merge
                assert det_to_track[track_to_det_merge[i]] != -1  # noqa: S101
            ...
            if track_to_det[i] != -1 and det_to_track_merge[track_to_det[i]] != -1:  # First branch of a merge
                ...
                if other.track_state != TrackHandler.TrackState.HYPOTHETICAL:
                    if track.track_state == TrackHandler.TrackState.HYPOTHETICAL:
                        ...
                        track = other  # Will be updated and kept in active  # noqa: PLW2901
    ...
    def _remove_single_split(self, tracks: list[byotrack.Track]) -> list[byotrack.Track]:
        ...
        for identifier in [identifier for identifier in splits if len(splits[identifier]) == 1]:
            ...
            if parent.detection_ids is not None:
                assert child.detection_ids is not None, "Incompatible track format"  # noqa: S101
    ...
    def _undo_split(self, track: TrackHandler, still_active: list[TrackHandler]) -> None:
        ...
        parent = next(other for other in self.inactive_tracks if other.identifier == track.parent_id)
        child = next(other for other in self.active_tracks + self.inactive_tracks if other.parent_id == track.parent_id)

        # Remove parent from inactive tracks
        self.inactive_tracks.remove(parent)

        if child.is_active():  # noqa: SIM108
    ...
    def _handle_extra_detections(self, detections: byotrack.Detections) -> None:
    ...
    def associate(self, frame: np.ndarray, detections: byotrack.Detections) -> torch.Tensor:
        ...
        self._unmatched_detections = torch.full((len(detections),), fill_value=True)
        ...
        unmatched_tracks = torch.full((len(self.active_tracks),), fill_value=True)
    ...
    def update(self, frame: np.ndarray, detections: byotrack.Detections) -> None:
        ...
        if len(self.all_positions[-1]) != len(self.active_tracks):
            raise RuntimeError("`post_association` should have registered the positions of each active tracks")
