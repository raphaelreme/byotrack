...
class DirectedPropagate(Protocol):
    ...
    def __call__(  # noqa: D102
        self, tracks_matrix: torch.Tensor, video: Sequence[np.ndarray] | np.ndarray, *, forward=True, **kwargs: Any
    ) -> torch.Tensor: ...
...
def forward_backward_propagation(
    ...
    method: str | DirectedPropagate,
    **kwargs: Any,
) -> torch.Tensor:
    """Fill all NaN values in the tracks matrix by merging a forward and backward propagation.

    Estimate missing positions for any defined track and frame. First it computes forward estimations,
    ...
    forward_positions = directed_propagate(tracks_matrix, video, forward=True, **kwargs)
    backward_positions = directed_propagate(tracks_matrix, video, forward=False, **kwargs)
...
def merge(
    ...
    if both.any():
        ...
        for i in range(1, tracks_matrix.shape[0]):
            ...
            i = tracks_matrix.shape[0] - i - 1  # noqa: PLW2901
...
def constant_directed_propagate(
    tracks_matrix: torch.Tensor,
    video: Sequence[np.ndarray] | np.ndarray = (),  # noqa: ARG001
    *,
    forward=True,
    **kwargs: Any,
) -> torch.Tensor:
    ...
    """
    if kwargs:
        warnings.warn(f"`constant_directed_propagate` has been given unused kwargs: {kwargs}", stacklevel=2)
...
def tps_directed_propagate(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        forward (bool): Forward or backward propagation
            Default: True (Forward)
        alpha (float): Thin Plate Spline regularization (See `torch_tps` library). We advise to use alpha > 5.0.
    ...
    """
    if kwargs:
        warnings.warn(f"`tps_directed_propagate` has been given unused kwargs: {kwargs}", stacklevel=2)
    ...
    for i in tqdm.trange(1, tracks_matrix.shape[0], desc=f"TPS {'forward' if forward else 'backward'} propagation"):
        ...
        if control_mask.sum() < 5:  # noqa: PLR2004
            warnings.warn(f"Too few control points while propagating ({control_mask}.sum())", stacklevel=2)
...
def optical_flow_directed_propagate(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        forward (bool): Forward or backward propagation
            Default: True (Forward)
        optflow (byotrack.OpticalFlow | None): Optical flow to use
            Default: None, i.e. default to DummyOpticalFlow (predict no displacement <=> constant propagate)
        kwargs (Any): Unused, added for api purposes.
    ...
    """
    if kwargs:
        warnings.warn(f"`optical_flow_directed_propagate` has been given unused kwargs: {kwargs}", stacklevel=2)

    if optflow is None:
        optflow = DummyOpticalFlow()
