...
class RTSSmoother(byotrack.Refiner):
    ...
    In order to have consistent results, the tracks positions should be extracted as the positions of an independent
    ...
    def run(self, _: Sequence[np.ndarray] | np.ndarray, tracks: Collection[byotrack.Track]) -> list[byotrack.Track]:
        ...
        is_defined = torch.full((positions.shape[0], positions.shape[1]), fill_value=False)
        offsets: list[int] = []
        ...
        for i, track in enumerate(tracks):
            ...
            if offset != 0:  # Handle tracks starting by NaN values
                warnings.warn("A track is starting with NaN values. It will be clipped.", stacklevel=2)
        ...
        was_defined = torch.full((positions.shape[1],), fill_value=False)
        for frame_id in tqdm.trange(positions.shape[0], desc=self.description_forward):
            ...
            # Initialization
            assert measured[starting].all()  # Always measured on start point  # noqa: S101
