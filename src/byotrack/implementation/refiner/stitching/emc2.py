...
import byotrack
from byotrack.implementation.refiner import propagation
from byotrack.implementation.refiner.stitching import dist_stitcher
...
def _fast_emc2_dist(propagation_matrix: np.ndarray, skip_mask: np.ndarray, ranges: np.ndarray) -> np.ndarray:
    ...
    for i in numba.prange(n_tracks):
        # Could set some verbosity (without parallel)
        # with numba.objmode():
        #     print(f"Computing emc2 dist {i+1} / {n_tracks}", flush=True, end="\r")  # noqa: ERA001
        for j in numba.prange(n_tracks):
            if skip_mask[i, j]:
                continue
...
class EMC2Stitcher(dist_stitcher.DistStitcher):
    ...
    Attributes:
        ...
        kwargs (dict[str, Any]): Additional parameters given to `method`.
            ...
            interpolation (see `tps_propation` module and `torch_tps` library). We advise to use alpha > 5.0
    ...
    def __init__(self, method: str | propagation.DirectedPropagate = "tps", eta: float = 5.0, **kwargs: Any) -> None:
    ...
    def compute_dist(self, video: Sequence[np.ndarray] | np.ndarray, tracks: Collection[byotrack.Track]) -> np.ndarray:
