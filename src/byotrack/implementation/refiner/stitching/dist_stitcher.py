...
Dist = Callable[[Sequence[np.ndarray] | np.ndarray, Collection[byotrack.Track]], np.ndarray]
...
class DistStitcher(byotrack.Refiner):
    ...
    def merge(tracks: Collection[byotrack.Track], links: np.ndarray) -> list[byotrack.Track]:
        ...
        print(f"Merging {len(tracks)} tracks into {len(connected_components)} resulting tracks")  # noqa: T201
        ...
        for connected_component in connected_components:
            ...
            merge_ids = {track_list[i].merge_id for i in connected_component}
            parent_ids = {track_list[i].parent_id for i in connected_component}
            if len(merge_ids) > 2 or len(parent_ids) > 2:  # noqa: PLR2004  # -1 is included in the set
                raise RuntimeError("Unable to stitch merging/splitting tracks")
    ...
    def skip_computation(
        ...
        4. i is not split or merged to any other tracks.
        ...
        Args:
            ...
            max_dist (float): Cannot stitch track i and track j if the last position of i and
                first position of j are farther than `max_dist` (ignored if max_dist <= 0)
            max_gap (int): Cannot stitch track i and track j if i ended more
        ...
        merge_in = torch.full((len(tracks),), fill_value=False)
        ...
        split_out = torch.full((len(tracks),), fill_value=False)
...
def _extract_connected_components(n: int, edges: dict[int, int]) -> list[set[int]]:
    ...
    """
    edges = edges.copy()
    seen_nodes = np.full(n, fill_value=False)
    ...
    for node in range(n):
        ...
        while node in edges:  # Pop each edges from the start node a follow them
            node = edges.pop(node)  # noqa: PLW2901
            seen_nodes[node] = True
