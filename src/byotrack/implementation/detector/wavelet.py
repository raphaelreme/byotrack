...
_DIM_TO_CONV: dict[int, type[torch.nn.Conv1d | torch.nn.Conv2d | torch.nn.Conv3d]] = {
    1: torch.nn.Conv1d,
    2: torch.nn.Conv2d,
    3: torch.nn.Conv3d,
}
...
class B3SplineUWT(torch.nn.Module):
    r"""Undecimated Wavelet Transform with B3Spline for nD images.
    ...
    def __init__(self, level=3, dim=2, *, return_all=True):
        ...
        for layer in self.layers:
            layer = cast("torch.nn.Conv1d | torch.nn.Conv2d | torch.nn.Conv3d", layer)
            layer.weight.requires_grad_(False)  # noqa: FBT003  (torch seems to miss behave here)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if len(spatial) != self.dim:
            raise ValueError(f"The module was constructed for {self.dim}D images. Input is {len(spatial)}D.")
...
class B3SplineUWTApprox1(torch.nn.Module):
    ...
    def __init__(self, level=3, dim=2, *, return_all=True):
        ...
        for layer in self.layers:
            ...
            layer.weight.requires_grad_(False)  # noqa: FBT003
        ...
        for i in range(level):
            ...
            for j in range(dim):
                conv = conv_builder(
                    1,
                    1,
                    tuple(5 if k == j else 1 for k in range(dim)),  # type: ignore[arg-type]
                    dilation=tuple(2**i if k == j else 1 for k in range(dim)),  # type: ignore[arg-type]
                    padding=tuple(2 ** (i + 1) if k == j else 0 for k in range(dim)),  # type: ignore[arg-type]
                    bias=False,
                    padding_mode="reflect",
                )
                conv.weight.requires_grad_(False)  # noqa: FBT003
    ...
    * Thresholding -> We follow the original paper using k times the std
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.b3swt: B3SplineUWT | B3SplineUWTApprox1 | B3SplineUWTApprox2
    ...
    def optimize(  # noqa: C901, PLR0912, PLR0915
        ...
        *,
        repeat=5,
        warm_up=True,
        splines=(B3SplineUWT, B3SplineUWTApprox1, B3SplineUWTApprox2),
    ) -> WaveletDetector:
        ...
        on some gpus when deterministic was set to False for 3D conv.

        Depending on your hardware, kernels and pytorch version, one solution may be better than the other.
        ...
        if self.device.type == "cpu":
            for model in models:
                ...
                if t > 5 * best_time:
                    print(f"CPU: {model.__class__.__name__} computed in {t} => skip")  # noqa: T201
                    continue

                total_time /= repeat

                print(f"CPU: {model.__class__.__name__} compute in avg of {total_time}")  # noqa: T201
            ...
            if best_cudnn and not old_cudnn:
                warnings.warn(f"Enabling cudnn backend (deterministic={best_deterministic})", stacklevel=2)
            if not best_cudnn and old_cudnn:
                warnings.warn("Disabling cudnn backend", stacklevel=2)
            if old_deterministic != best_deterministic:
                warnings.warn(f"Switching cudnn deterministic to {deterministic}", stacklevel=2)

            torch.backends.cudnn.enabled = best_cudnn
            torch.backends.cudnn.deterministic = best_deterministic

        print(f"Selecting {best_model.__class__.__name__}")  # noqa: T201
        self.b3swt = best_model
