...
class Watershed:
    ...
    def __init__(
        self,
        maximum_footprint: np.ndarray | None = None,
        maximum_dilation: np.ndarray | None = None,
    ...
    def watershed(self, mask: np.ndarray, source_map: np.ndarray | None = None) -> np.ndarray:
        ...
        Args:
            ...
            source_map (np.ndarray | None): "Intensity" value for each pixel of the mask. Local maxima are
        ...
        if source_map.shape != mask.shape:
            raise ValueError("source_map.shape != mask.shape (channels are not expected in source_map)")

        if mask.dtype != np.bool_:
            mask = mask > 0

        peak_mask = self.peak_mask(source_map, mask)
        markers, _ = ndi.label(peak_mask, self._default_footprint(mask.ndim))  # type: ignore[misc]
...
class WatershedRefiner(byotrack.DetectionsRefiner):
    ...
    def apply(self, detections, frame=None):
        ...
        if self.mode == self.Mode.INTENSITY:
            if frame is None:
                warnings.warn("Frame was not provided for mode INTENSITY. Fall back to EDT", stacklevel=2)
