...
_colors = [tuple(int(c * 255) for c in x[:3]) for x in map(_hsv, [i / 200 for i in range(200)])]
...
def _convert_to_uint8(frame: np.ndarray) -> np.ndarray:
    ...
    elif frame.max() > 255:  # noqa: PLR2004
        warnings.warn(
...
def display_lifetime(tracks: Collection[byotrack.Track]) -> None:
...
def temporal_projection(
    ...
    *,
    ...
    Args:
        ...
        background (np.ndarray | None): Optional frame to display behind the tracks.
    ...
    if background is not None:
        if background.ndim == 2:  # noqa: PLR2004
    ...
    for track_i, points in enumerate(tracks_tensor.permute(1, 0, 2)):
        ...
        points = points[is_defined[:, track_i]] - offset  # noqa: PLW2901

        if color_by_time:
            valid_frames = frames[is_defined[:, track_i]]

            previous = points[0]
            for frame_id, point in zip(valid_frames[1:], points[1:], strict=True):
                cv2.line(
                    image,
            ...
            cv2.polylines(
                ...
                isClosed=False,
                color=colors[track_i % len(colors)],
            )
...
class InteractiveVisualizer:
    """Interactive visualization with Open-CV.
    ...
    def __init__(
        ...
        self._stack_id = 0 if len(self.frame_shape) != 3 else self.frame_shape[0] // 2  # noqa: PLR2004
    ...
    def _run(self, fps=20) -> None:
        ...
        while True:
            ...
            if self._display_video and len(self.video) != 0:
                ...
                _frame = _frame[self._stack_id] if len(self.frame_shape) == 3 else _frame  # noqa: PLR2004
            ...
            if self.scale != 1:
                frame = cv2.resize(  # type: ignore[assignment]
            ...
            if len(self.frame_shape) == 3:  # noqa: PLR2004
    ...
    def draw_segmentation(self, frame: np.ndarray) -> np.ndarray:
        ...
        if self.detections_sequence[self._frame_id].dim == 3:  # noqa: PLR2004
    ...
    def draw_tracks(self, frame: np.ndarray) -> np.ndarray:
        ...
        for track in self.tracks:
            ...
            if len(point) == 3:  # noqa: PLR2004
                if (point[0] / self.scale - self._stack_id).abs() > 5:  # noqa: PLR2004
    ...
    def handle_actions(self, key: int) -> bool:  # noqa: C901, PLR0912
        ...
        if len(self.frame_shape) == 3 and key == ord(self.keys["DOWN"]):  # noqa: PLR2004
            self._stack_id = (self._stack_id - 1) % self.frame_shape[0]

        if len(self.frame_shape) == 3 and key == ord(self.keys["UP"]):  # noqa: PLR2004
            self._stack_id = (self._stack_id + 1) % self.frame_shape[0]
    ...
    def _get_frame_shape(self) -> tuple[int, ...]:
        ...
        if self.detections_sequence:
            if dim:  # Shape is already defined from the video
                ...
                if self.detections_sequence[0].dim > dim:
                    raise ValueError("Unable to handle 3D detections with 2D videos")
            else:
                ...
                def _reduce(reduced: tuple[int, ...], shape: tuple[int, ...]) -> tuple[int, ...]:
                    return tuple(max(s_r, s) for s_r, s in zip(reduced, shape, strict=True))
        ...
        if self.tracks:
            ...
            if (minimum < 0).any() or (maximum[:dim] >= torch.tensor(frame_shape)[:dim]).any():
                print("Some tracks are out of frame")  # noqa: T201
    ...
    def _get_n_frames(self) -> int:
        ...
        n_frames = 1
        if self.tracks:
            n_frames = max(n_frames, *(track.start + len(track) for track in self.tracks))
...
class InteractiveFlowVisualizer:
    ...
    Attributes:
        ...
        precompute (bool): Compute all the flows first (bi directional) to prevent lagging
            Default: False
        tracks_colors (list[tuple[int, int, int]]): Colors to use for tracks
    ...
    def _run(self, fps=20) -> None:
        ...
        while True:
            ...
            if draw.shape[2] not in (1, 3):
                raise ValueError("We only support Grayscale images (C=1) or RGB images (C=3)")
...
def flow_to_rgb(flow: np.ndarray, clip: float | None = None) -> np.ndarray:
    ...
    Args:
        flow (np.ndarray): Flow to convert to rgb
            Shape: (2, H, W)
        clip (float | None): Clip the magnitude to this maximum value
            Default: None
