...
class CTCSoftwareRunner:
    """Runs CTC software (TRA/DET/SEG) [10].

    It wraps the TRA/DET/SEG software that are distributed at https://celltrackingchallenge.net/evaluation-methodology
    It is up to the user to download the correct executable and ensure that it works.

    These software take as inputs the path to the folder of the dataset (with ground-truth and results tracks),
    ...
    Note:
        This code requires CTC software as extra dependencies.
    ...
    def __init__(self, ctc_software: str | os.PathLike):
        ...
        if not self.ctc_software.is_dir():
            raise FileNotFoundError("Unable to find the unzipped CTC software (`ctc_software`)")

        if not (self.ctc_software / self.system).is_dir():
            raise FileNotFoundError(f"Unable to find the CTC executable for your system ({self.system})")

    def run(self, metric: str, dataset: str | os.PathLike, seq=1, n_digit=4) -> float:
        """Run the CTC software for the given metric and dataset.

        Args:
            metric (str): The metric to evaluate. One of (TRA, DET, SEG).
            dataset (str | os.PathLike): Path to the dataset to evaluate.
            seq (int): Sequence to evaluate inside the dataset.
                CTC software will compare {dataset}/{seq:02}_RES with {dataset}/{seq:02}_GT
                Default: 1
        ...
        """
        if metric not in ("SEG", "DET", "TRA"):
            raise ValueError("Only SEG, DET and TRA metrics are available with this software")
        ...
        print("Calling CTC software with:", cmd)  # noqa: T201
        outputs = subprocess.run(  # noqa: S602
...
class CTCMetrics(CTCSoftwareRunner):
    ...
    def compute_tracking_metric(
        ...
        ground_truth_tracks: str | os.PathLike | Collection[byotrack.Track],
        *,
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            ground_truth_tracks (str | os.PathLike | Collection[byotrack.Track]): Ground truth data.
    ...
    def compute_detection_metric(
        ...
        metric: str,
        detections_sequence: Sequence[byotrack.Detections],
        ground_truth_detections_sequence: str | os.PathLike | Sequence[byotrack.Detections],
    ) -> float:
        ...
        """
        if metric == "TRA":
            raise ValueError("TRA metric cannot be computed for detections")
    ...
    def copy_ground_truth(
        ground_truth_path: str | os.PathLike, target_path: str | os.PathLike, *, as_seg=False
    ) -> None:
        ...
        if len(gt_seg_tiff_paths) and len(gt_track_tiff_paths):
            warnings.warn(
                "Found both man_trackT.tif and man_segT.tif files in the GT folder. The most common ones will be used",
                stacklevel=2,
            )
        ...
        elif (ground_truth_path / "man_track.txt").exists():
        ...
        if is_seg and not is_res:
            for path in gt_seg_tiff_paths:
                ...
                if "_" in stem:
                    assert as_seg, "Cannot convert a partial SEG ground truth into a valid DET/TRA ones"  # noqa: S101
...
class BioMetrics:
    ...
    def __init__(self, fiji_path: str | os.PathLike) -> None:
        """Constructor.
    ...
    def run(self, dataset: str | os.PathLike, seq=1, n_digit=4) -> float:
        ...
        if len(groups) != 4:  # noqa: PLR2004
            raise RuntimeError(f"Expected 4 BIO metrics. Found {len(groups)}")

        self.last_metrics = tuple(max(float(group), 0.0) for group in groups)  # type: ignore[assignment]
