    ...
    for frame_id, detections in enumerate(detections_sequence):
        ...
        for label in range(detections.length):
            ...
            if detections.dim == 2:  # noqa: PLR2004
                ...
                # and then converted to the good string format: byte:byte:...:byte # TODO: Check the ruff fix
                ET.SubElement(roi, "boolMaskData").text = ":".join(hex(byte)[2:] for byte in compressed_bytes)  # noqa: FURB116
            else:
                ...
                for z in range(k, end_k):
                    ...
                    ET.SubElement(slice_, "boolMaskData").text = ":".join(hex(byte)[2:] for byte in compressed_bytes)  # noqa: FURB116
...
def load_tracks(path: str | os.PathLike) -> list[byotrack.Track]:
    ...
    if track_group is None:
        raise ValueError("No track found in the given file")
...
def save_tracks(tracks: Collection[byotrack.Track], path: str | os.PathLike, name: str = "ByoTrack") -> None:
    ...
    for track in tracks:
        ...
        for point in track.points:
            ...
            if point.shape[0] == 3:  # noqa: PLR2004
                x = str(point[2].item())
                y = str(point[1].item())
                z = str(point[0].item())
            else:
