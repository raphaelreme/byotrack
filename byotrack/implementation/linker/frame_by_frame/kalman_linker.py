...
class Cost(enum.Enum):
    ...
    LIKELIHOOD: Build the cost matrix to maximize the likelihood of the association.
    ...
    MAHALANOBIS: Mahalanobis distance, using the track uncertainty to correct the euclidean distance.
        The cost limit is expected to be the largest Mahalanobis distance to accept.
    MAHALANOBIS_SQ: Squared Mahalanobis distance.
    ...
    EUCLIDEAN_SQ: Squared euclidean distance.
...
class TrackBuilding(enum.Enum):
    ...
    DETECTION: Build tracks from detections without filtering nor filling gaps
    FILTERED: Build tracks from the Kalman filter outputs
    SMOOTHED: Add a backward run of the Kalman filter (RTS) to smooth tracks position
    """
...
class KalmanLinker(FrameByFrameLinker):
    ...
    def post_association(self, _: np.ndarray, detections: byotrack.Detections, links: torch.Tensor):
        ...
        # For velocity/acceleration, let's predict them at 0 but with thrice the process_noise
