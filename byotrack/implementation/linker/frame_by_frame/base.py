...
class AssociationMethod(enum.Enum):
    ...
    GREEDY: Select the best match between tracks and detections iteratively until
        no match can be selected below the cost limit eta. It is usually not optimal for tracking
        but it is much faster.
    OPT_HARD: Solve the linear association problem (see `pylapy`).
        Hard threshold the association matrix with the cost limit eta.
    OPT_SMOOTH: Solve a cost_limit extended association problem (see `pylapy`)
...
class TrackHandler:
    ...
    class TrackState(enum.IntEnum):
        ...
        HYPOTHETICAL: Initial state before validation of the track.
        VALID: The track has been validated and is still active
        FINISHED: The track is valid and finished
        INVALID: The track is not valid and deleted
